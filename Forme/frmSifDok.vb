...
Public Class frmSifDok
    ...
    Protected Overridable Sub inicijalizujPromenljive()
        ...
        strNaslov = "Šifarnik uslova prodaje "
        Me.Text = strNaslov
        Me.lblNaslov.Text = strNaslov



        strTabela = "FaktZUslovi"
        intID = 0
        strID = "FkzU_id"
        strSifra = "FkzU_Sifra" 'ovde unosim polje za povezivanje sa tabelom zaglavlje(bilo da je to povezivanje preko id ili sifre)

        strUpit = $"SELECT * FROM FaktZUslovi"

        strOrder = " ORDER BY " & strID

        intBrKol = 4
        intSirineKolona = New Integer() {50, 50, 100, 100}
        strNazKol = New String() {"ID", "Rb", "Sifra", "Opis"}
        strFildGrid = New String() {strID, "umesto rb", "FkzU_Sifra", "FkzU_Opis"}
        ...
        intBrTexBoxova = 2
        strFild = New String() {"FkzU_Sifra", "FkzU_Opis"}
        strLabelPoredText = New String() {"Šifra artikla", "Opis"}
        strInic = New String() {"", ""}
