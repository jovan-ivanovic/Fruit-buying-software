...
Public Class frmSifArtikli
    ...
    Protected Overridable Sub inicijalizujPromenljive()
        ...
        strNaslov = "Maloprodajni artikli "
        Me.Text = strNaslov
        ...
        intID = 0
        strID = "Kor_id"
        strSifra = "Kor_Sifra" 'ovde unosim polje za povezivanje sa tabelom zaglavlje(bilo da je to povezivanje preko id ili sifre)

        strUpit = $"SELECT * FROM   KontaR  WHERE Kor_Vrsta='MT' AND Kor_VrstaLookUp='MT'"

        strOrder = " ORDER BY " & strID

        intBrKol = 8
        intSirineKolona = New Integer() {50, 50, 100, 100, 100, 100, 100, 50}
        strNazKol = New String() {"ID", "Rb", "Sifra", "Naziv artikla", "JM", "Nabavna Cena", "PDV", "Grupa"}
        strFildGrid = New String() {strID, "umesto rb", "Kor_Sifra", "Kor_Opis", "Kor_jm", "Kor_CenaN", "Kor_Tar_Sifra", "Kor_Grupa"}
        ...
        intBrTexBoxova = 6
        strFild = New String() {"Kor_Sifra", "Kor_Opis", "Kor_jm", "Kor_Tar_Sifra", "Kor_CenaN", "Kor_Grupa"}
        strLabelPoredText = New String() {"Šifra artikla", "Naziv", "JM", "PDV", "Cena", "Šifra grupe"}
        strInic = New String() {"", "", "", "1", "0", ""}

        'inicijalizujem niz za izracunavanje sume
        intSuma = New Integer() {0, 0, 0, 0, 0, 0, 0, 0}
    ...
    Private Sub srediFormuPocetno()
        TextBox0.Text = modMain.vratiSledecuSifruUslov(strTabela, strSifra, "Kor_VrstaLookUp", "MT")
