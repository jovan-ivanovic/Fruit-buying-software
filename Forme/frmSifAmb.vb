...
Public Class frmSifOtkAmb
    ...
    Protected Overridable Sub inicijalizujPromenljive()
        ...
        strUpit = $"SELECT * FROM KontaR WHERE Kor_Vrsta='AB' AND Kor_VrstaLookUp='AB'"

        strOrder = " ORDER BY " & strID

        intBrKol = 5
        intSirineKolona = New Integer() {50, 50, 100, 100, 100}
        strNazKol = New String() {"ID", "Rb", "Sifra", "Naziv artikla", "Težina"}
        strFildGrid = New String() {strID, "umesto rb", "Kor_Sifra", "Kor_Opis", "Kor_Tezina"}
        ...
        intBrTexBoxova = 3
        strFild = New String() {"Kor_Sifra", "Kor_Opis", "Kor_Tezina"}
        strLabelPoredText = New String() {"Šifra artikla", "Naziv", "Težina"}
        strInic = New String() {"", "", ""}
    ...
    Private Sub srediFormuPocetno()
        TextBox0.Text = modMain.vratiSledecuSifruUslov(strTabela, strSifra, "Kor_VrstaLookUp", "AB")
